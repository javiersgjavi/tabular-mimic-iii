...
def save_data(path, data):
    for name, array in zip(['x_dl', 'x_ml', 'y'], data):
...
def preprocess(data, bins, measures_values, imputation_method, set):
    ...
    for i in tqdm(range(values_dim.shape[0]), desc=f'Preprocessing {set} set'):
        ...
        elif imputation_method == 'nan_imputation':
            row_preprocessed = nan_imputation(row)
...
def preprocess_general(train_data, val_data, test_data, imputation_method):
    ...
    train_path = f'./data/{imputation_method}/train/'
    val_path = f'./data/{imputation_method}/val/'
    test_path = f'./data/{imputation_method}/test/'
...
def preprocess_gp(train_data, val_data, test_data):
    ...
    train_path = f'{path_start}/train/'
    val_path = f'{path_start}/val/'
    test_path = f'{path_start}/test/'
